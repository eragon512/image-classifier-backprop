...
void fetch_dataset(const string& imglistfile,vector< pair< vector<float>,vector<float> > >& bpnn_train) {
	...
	for(int q=0;q<imglist.size();++q) {
		bpnn_train.push_back(make_pair(read_pmg_normalised(imglist[q]),get_name_vector(imglist[q])));
	}
}

void randomise_training_set(vector< pair< vector<float>,vector<float> > >& bpnn_train) {
	random_device rd;
	mt19937 g(rd());
	shuffle(bpnn_train.begin(), bpnn_train.end(), g);
}
...
void train_network(const string &imglistfile) {
	...
	for(int e=0;e<epochs;++e) {
		...
		randomise_training_set(bpnn_train);
		...
		for(int l=0;l<len;++l) {
			...
			accuracy += classify_name(o_out,no_output,y,no_output)/(double)len;
			//cout<<"Actual Name: "<<"\n";
...
int main() {
	initialise_weights();
	train_network("dataset/lists/straighteven_train.list");
	test_network("dataset/lists/straighteven_test1.list");
	test_network("dataset/lists/straighteven_test2.list");
	return 0;
}
