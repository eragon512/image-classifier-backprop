...
void load_instance(vector<float>& in,vector<float> &out) {
	...
	//for(int i=0;i<out.size();++i) printf("%f ",y[i]); printf("\n");
}
...
void fetch_dataset(const string& imglistfile,vector< pair< vector<float>,vector<float> > >& bpnn_train) {
	...
	for(int q=0;q<imglist.size();++q) {
		bpnn_train.push_back(make_pair(read_pmg_normalised(imglist[q]),get_pose_vector(imglist[q])));
	}
}
...
int main() {
	initialise_weights();
	train_network("dataset/lists/all_train.list");
	test_network("dataset/lists/all_test1.list");
	test_network("dataset/lists/all_test2.list");
	return 0;
}
